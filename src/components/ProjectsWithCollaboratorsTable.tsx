...
export default function ProjectsWithCollaboratorsTable({ projects }: { projects: ProjectCollab[]; }) {
  return (
    <div className="mt-16">
    <table className="font-mono leading-8">
      <thead>
        <tr className='text-left border-b'>
          ...
          <th className='pr-8'>person_name</th>
          <th>date_created</th>
        </tr>
      </thead>
      <tbody>
        {projects.map(({ project_id, project_number, title, folder_path, name, date_created }) => (
          <tr key={project_id}>
            ...
            <td className='text-nowrap pr-8'>{name}</td>
