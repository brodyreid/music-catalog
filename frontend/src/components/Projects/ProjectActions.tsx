      ...
      <div className={`grid grid-cols-3 gap-x-8 p-8 pt-6 border border-neutral-500 rounded-lg ${!state.current ? 'pointer-events-none opacity-50' : ''}`}>
        ...
        <div>
          ...
          <div className='flex flex-col'>
            <TextArea
              value={state.notes ?? ''}
              onChange={(event) => dispatch({ type: 'changed_notes', notes: event.target.value })}
        ...
        <div>
          ...
          <SelectField
            ...
            onChange={(event) => dispatch({ type: 'changed_musical_key', musical_key: event.target.value as MusicalKey ?? null })}
          >
            <option>{null}</option>
